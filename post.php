...
function getComments ($id) {
    $sql = "SELECT * FROM comment INNER JOIN users ON comment.user_id =  users.id WHERE user_id = $id";
    return requestDb($sql);
};
...
$page_content = include_template('/post_main.php', ['post' => getPost($ind), 'post_text' => $post_text, 'post_video' => $post_video, 'post_photo' => $post_photo, 'post_quote' => $post_quote, 'post_link' => $post_link, 'comments' => getComments($ind), 'publication_count' => publicationCount($ind)]);
